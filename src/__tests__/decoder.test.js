...
describe('decoder', () => {
  ...
  test('throw error: decoder() input type check', async () => {
    ...
    ).rejects.toThrowError('decoder(): precisionValue must be of type number');

    await expect(
      decoder(50, encoding.compressionMethod, testMZ),
    ).rejects.toThrowError('decoder(): precisionValue defined not valid');
    ...
    ).rejects.toThrowError('decoder(): compressionMethod defined not valid');

    // encodedData
    await expect(
      decoder(encoding.precisionValue, encoding.compressionMethod, 0),
    ).rejects.toThrowError('decoder(): encodedData must be of type string');
  });
  ...
  test('throw error: encoder() input type check', async () => {
    ...
    ).rejects.toThrowError('encoder(): precisionValue must be of type number');

    await expect(
      encoder(50, encoding.compressionMethod, testMZ),
    ).rejects.toThrowError('encoder(): precisionValue defined not valid');
    ...
    ).rejects.toThrowError('encoder(): compressionMethod defined not valid');
